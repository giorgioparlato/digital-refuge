...
**Can I still switch blocking off, or uninstall, mid-session?** Not in two taps. During a session, the Settings screens that switch blocking off, and the uninstall prompt, are covered by a screen that points you back to the unlock steps (you can turn this off in settings → locking blocking down). The deliberate routes stay open: end the session through the unlock steps, or restart the phone in safe mode. And if blocking does go off, Do Not Disturb, held notifications and greyscale keep going until the session ends, so it only unblocks apps.
